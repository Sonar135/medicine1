...
<?php
    ...
    if(isset($_POST['signup'])){
        ...
        if(empty_ad_signup($email, $fname, $phone, $password, $confirm)!== false){
            
            
            header("location: admin_auth.php?error=emptyfield");
            exit();
 
        }

        if(invalid_email($email)!== false){
            header("location: admin_auth.php?error=invalidemail");
        //     echo '<div class="message" id="message">
        //     error: INVALID EMAIL
        // </div>';
        exit();
        ...
        if (invalid_password($password)) {
            header("location: admin_auth.php?error=invalid_pass");
            exit();
        ...
        if(pwd_match($password, $confirm)!== false){
            ...
            header("location: admin_auth.php?error=pwd_not_match");
            exit();
        }

        if(admin_email_exists($conn, $email)!== false){
            header("location: admin_auth.php?error=email_in_use");
      
            exit();
        }

        create_admin($conn, $email, $fname,  $phone, $password, $confirm);
...
<?php
    ...
    if(emptylogin($email, $password)){
        header("location: admin_auth.php?error=empty_login");
        exit();
    }
...
<div class="img_container">
    ...
    <div class="container auth">
           ...
           <form action="" method="post" >  <div class="right">
                    ...
                    <button name="signup" id="sign_up_btn">sign up</button>
